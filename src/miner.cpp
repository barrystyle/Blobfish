...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("BLOBFISHXMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("BLOBFISHXMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("BLOBFISHXMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //LogPrintf("Running BLOBFISHXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("BLOBFISHXMiner exception");
    } catch (...) {
        LogPrintf("BLOBFISHXMiner exception");
    }

    LogPrintf("BLOBFISHXMiner exiting\n");
}
