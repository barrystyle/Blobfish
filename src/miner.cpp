...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zPEPPAPOW spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zPEPPAPOW spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zPEPPAPOW serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("PEPPAPOWXMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("PEPPAPOWXMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("PEPPAPOWXMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //LogPrintf("Running PEPPAPOWXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("PEPPAPOWXMiner exception");
    } catch (...) {
        LogPrintf("PEPPAPOWXMiner exception");
    }

    LogPrintf("PEPPAPOWXMiner exiting\n");
}
