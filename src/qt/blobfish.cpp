...
/** Class encapsulating BLOBFISHX Core startup and shutdown.
...
/** Main BLOBFISHX application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (BLOBFISHXGUI)
...
{
    window = new BLOBFISHXGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &BLOBFISHXGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &BLOBFISHXGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(BLOBFISHXGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(BLOBFISHXGUI::DEFAULT_WALLET);
        ...
        // BLOBFISHX: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BLOBFISHXGUI::handlePaymentRequest);
        connect(window, &BLOBFISHXGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
