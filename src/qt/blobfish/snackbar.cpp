...
#include "qt/peppapow/forms/ui_snackbar.h"
#include "qt/peppapow/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(BLOBFISHXGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &BLOBFISHXGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
