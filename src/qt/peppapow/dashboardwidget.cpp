...
DashboardWidget::DashboardWidget(PEPPAPOWXGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &PEPPAPOWXGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &PEPPAPOWXGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &PEPPAPOWXGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("PEPPAPOW")
    updateDisplayUnit();
}
