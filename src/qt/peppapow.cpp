...
/** Class encapsulating PEPPAPOWX Core startup and shutdown.
...
/** Main PEPPAPOWX application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (PEPPAPOWXGUI)
...
{
    window = new PEPPAPOWXGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &PEPPAPOWXGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &PEPPAPOWXGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(PEPPAPOWXGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(PEPPAPOWXGUI::DEFAULT_WALLET);
        ...
        // PEPPAPOWX: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PEPPAPOWXGUI::handlePaymentRequest);
        connect(window, &PEPPAPOWXGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. PEPPAPOW can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("PEPPAPOW Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("PEPPAPOW Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("PEPPAPOW Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
