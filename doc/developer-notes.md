...
- [Development guidelines](#development-guidelines)
    - [General PEPPAPOWX Core](#general-peppapow-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
PEPPAPOWX Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play PEPPAPOWs (tPEPPAPOW)" on the test network, if you
...
PEPPAPOWX Core is a multi-threaded application, and deadlocks or other
...
- BitcoinMiner : Generates PEPPAPOWs (if wallet is enabled).
...
However, in open source software such as PEPPAPOWX Core, where everyone uses
...
pay attention to for reviewers of PEPPAPOWX Core code.

General PEPPAPOWX Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: PEPPAPOWX Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against PEPPAPOWX Core so that they can be integrated
...
file descriptors it will cause problems with PEPPAPOWX's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:PEPPAPOWX-Project/PEPPAPOWX.git
